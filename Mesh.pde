...
class MESH 
{
  ...
  MESH() {
    for (int i=0; i<maxnv; i++) G[i]=new PNT();
  }
  ...
  void resetVisitedTriangles() {
    for (int t=0; t<nt; t++) visitedTriangles[t]=false;
  }

  void loadVertices(PNT[] P, int n) {
    nv=0; 
    for (int i=0; i<n; i++) addVertex(P[i]);
  }
  void writeVerticesTo(PNTS P) {
    for (int i=0; i<nv; i++) P.G[i].setTo(G[i]);
  }
  void addVertex(PNT P) { 
    G[nv++].setTo(P);
  ...
  PNT LocationOfVertexOfCorner (int c) {
    return G[V[c]];
  }                             // shortcut to get the point where the vertex v(c) of corner c is located
  PNT LocationOfCornerDot(int c) {
    return P(0.6, LocationOfVertexOfCorner(c), 0.2, LocationOfVertexOfCorner(p(c)), 0.2, LocationOfVertexOfCorner(n(c)));
  }   // computes offset location of point at corner c

  boolean isBeachFacing(int c) {
    return(O[c]==c);
  };  // a border (beach-facing) corner
  boolean isNotBeachFacing(int c) {
    return(O[c]!=c);
  };  // not a border corner
  ...
  void printInfo() {
    println("c = "+c+", nc = "+nc+", 3nt = "+3*nt+", nt = "+nt+", nv="+nv);
  }

  // DISPLAY
  void showCurrentCorner(float r) { 
    if (isBeachFacing(c)) cwf(black, 1, red); 
    else cwf(black, 1, grey); 
    show(LocationOfCornerDot(c), r);
  };   // renders corner c as small ball
  void showEdgeFacingCorner(int c) {
    w(rt); 
    show(LocationOfVertexOfCorner(p(c)), LocationOfVertexOfCorner(n(c)));
  };  // draws edge of t(c) opposite to corner c
  void showVertices(float r) // shows all vertices green/yellow inside, red outside
  {
    for (int v=0; v<nv; v++) 
    {
      if (isInterior[v]) cwf(dgreen, 2, yellow); 
      else cwf(dred, 2, red);
      show(G[v], r);
    }
  }                          
  ...
  void showEdges() {
    for (int i=0; i<nc; i++) if (i<=o(i)) showEdgeFacingCorner(i);
  };         // draws all edges of mesh twice
  void showBorderEdges() {
    for (int i=0; i<nc; i++) {
      if (isBeachFacing(i)) {
        showEdgeFacingCorner(i);
      };
    };
  ...
  void computeDelaunayTriangulation() {     // performs Delaunay triangulation using a quartic algorithm
    ...
    for (int i=0; i<nv-2; i++) for (int j=i+1; j<nv-1; j++) for (int k=j+1; k<nv; k++)
      ...
      {    
        ...
        if (!found) if (ccw(G[i], G[j], G[k])) addTriangle(i, j, k); 
  ...
  PNT triCenter(int c) {
    return P(LocationOfVertexOfCorner(c), LocationOfVertexOfCorner(n(c)), LocationOfVertexOfCorner(p(c)));
  }  // returns center of mass of triangle of corner c

  PNT triCircumcenter(int c) {
    return CircumCenter(LocationOfVertexOfCorner(c), LocationOfVertexOfCorner(n(c)), LocationOfVertexOfCorner(p(c)));
  ...
  int cornerIndexFromVertexIndex(int v) {
    for (int c=0; c<3*nt; c++) if (v(c)==v) return c; 
    return -1;
  } 



  //********************************************************  PHASE A FOR 3451 PROJECT 4 2020 ********************************************************
  ...
  //******************************************************** PHASE B FOR 3451 PROJECT 4 2020 ********************************************************
  ...
  {    
    ...
    for (int i = 0; i < nc; i++) {
      if (isInterior[v(i)]) {
        W[v(i)].add(V(LocationOfVertexOfCorner(i), LocationOfVertexOfCorner(n(i))));
        neighbors[v(i)] += 1;
      }
    }
    for (int j = 0; j < nv; j++) {
  ...
  {    
    ...
    float sumOfWeights [] = new float [nv]; //contains sum of all weighted edges for a corresponding vertex
    for (int i = 0; i < nc; i++) {
      if (isInterior[v(i)]) {
        VCT e = V(LocationOfVertexOfCorner(n(i)), LocationOfVertexOfCorner(p(i)));
        //sumOfWeights[v(i)] += sqrt(dot(e, e));
        sumOfWeights[v(i)] += e.norm();
        VCT v = V(LocationOfVertexOfCorner(i), P(LocationOfVertexOfCorner(n(i)), LocationOfVertexOfCorner(p(i)))); //edge from vertex to midpoint
        //W[v(i)].add(v.scaleBy(sqrt(dot(e, e)))); //scale by edge length
        W[v(i)].add(v.scaleBy(e.norm()));
      }
    }

    for (int j = 0; j < nv; j++) {
      if (isInterior[j]) {
        G[j].translate((ratio/sumOfWeights[j]), W[j]);
      }
    }
  }

  //******************************************************** PHASE C FOR 3451 PROJECT 4 2020 ********************************************************
  ...
  {
    int next = n(cc);
    while (isNotBeachFacing(next)) {
      next = n(o(next));
    }
    return next;
  }

  void smoothenBorderWithTuck(float ratio) { // even  border vertiex locations
    VCT[] W = new VCT [nv];    // correction vectors
    for (int i = 0; i < nv; i++) {
      W[i] = V(0, 0);
    } //initialize W
    for (int i = 0; i < nc; i++) {
      if (isBeachFacing(i)) {
        int a = n(leftBeachNeighbor(i));
        int b = n(rightBeachNeighbor(i));
        int c = n(i);
        PNT avg = P(LocationOfVertexOfCorner(a), LocationOfVertexOfCorner(b));
        W[v(c)].add(V(LocationOfVertexOfCorner(c), avg));
      }
    }
    for (int j = 0; j < nc; j++) {
      if (isBeachFacing(j)) {
        G[v(n(j))].translate(W[v(n(j))].scaleBy(ratio));
      }
    }
  }

  void smoothenBorderWithCubicPredictor(float ratio) { // even border vertiex locations
    VCT[] W = new VCT [nv];    // correction vectors
    // PROJECT 4 PHASE C (EXTRA):  STUDENT SHIULD PROVIDE THIS CODE
    for (int i = 0; i < nv; i++) {
      W[i] = V(0, 0);
    } //initialize W
    for (int i = 0; i < nc; i++) {
      if (isBeachFacing(i)) {
        int a = n(leftBeachNeighbor(leftBeachNeighbor(i))); 
        int b = n(leftBeachNeighbor(i)); 
        int c = n(i); 
        int d = n(rightBeachNeighbor(i)); 
        int e = n(rightBeachNeighbor(rightBeachNeighbor(i))); 
        PNT avg = L(L(LocationOfVertexOfCorner(a), (9/8), LocationOfVertexOfCorner(b)), .5, L(LocationOfVertexOfCorner(e), (9/8), LocationOfVertexOfCorner(d)));
        W[v(c)].add(V(LocationOfVertexOfCorner(c), avg));
      }
    }
    for (int j = 0; j < nc; j++) {
